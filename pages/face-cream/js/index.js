...
(function(window, $) {
    ...
    var SwiperManager = {
      ...
      initJQuerySlide: function() {
        // jQuery跑马灯
        if ($.fn.slide) {
          // 先销毁已存在的实例
          var $scroll2 = jQuery(".sp-scroll2");
          if ($scroll2.data('slide')) {
            $scroll2.slide('destroy');
          }
          // 重新初始化
          $scroll2.slide({
            mainCell: ".nav-imgs",
    ...
    var ResponsiveLayout = {
      ...
      updateSwipers: function() {
        // 延迟执行，确保zoom变化已完成
        var self = this;
        setTimeout(function() {
          // 只更新已存在的 Swiper 实例，不重新初始化
          for (var key in SwiperManager.swipers) {
            if (SwiperManager.swipers.hasOwnProperty(key)) {
              var swiper = SwiperManager.swipers[key];
              // 检查 swiper 是否存在、已初始化且具有 update 方法
              if (swiper && swiper.initialized !== false && typeof swiper.update === 'function') {
                try {
                  swiper.update();
                } catch (e) {
                  console.warn('Swiper update failed for', key, e);
                }
              }
            }
          }
          // 重新初始化 jQuery slide 跑马灯
          SwiperManager.initJQuerySlide();
        }, 50);
      },
